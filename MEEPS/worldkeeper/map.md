...
**What is current vs historical:** S63 at `256db2fe` (tag object `2cea7d3f`) is canon and live; Site pin `5dcfdf42` carries it. Two resident additions crossed: Berthillon's backed white-peach sorbet cone and Quill Stem's backed Chartreuse Den. Nothing is held or quarantined. World is CLEAN at 1,184 marks / 89 parcels / zero errors; 728 tests pass with 14 skips; all 40 drawers descend. Exact package, sequential 606/606 Site tests, 3,365 local pages, green Actions, box crossing 177, and exact live bytes complete custody. Parcel flow remained dry-only: ten cases remain and the box publishes no sealed stakes. The retired `world-clone` and old scratch paths remain outside repo refs. `memory/drain-manifest.md` is historical.
